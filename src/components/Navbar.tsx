        ...
        <div className="flex items-center gap-4">
          {!isHomePage ? (
            ...
            <Link href="/" className="group flex items-center gap-3">
              <div className={`flex h-10 w-10 items-center justify-center rounded-full transition-all duration-500 ${shouldShowDarkText ? 'bg-navy text-white' : 'bg-white/20 backdrop-blur-sm text-white'}`}>
                <Ship className={`h-5 w-5 transition-transform duration-500 group-hover:rotate-12 ${!shouldShowDarkText && isHomePage ? 'text-gold' : ''}`} />
              </div>
              <span className={`text-sm font-bold tracking-tighter uppercase md:text-xl transition-colors duration-500 ${shouldShowDarkText ? 'text-navy' : 'text-white'}`}>
